...
`src/data/questions.json` 现已替换为出题人提供的完整版 150 题，文件保留 `meta` 与 `questions` 外层结构。150 道题全部标记为 `enabled: true`、`verified: true`，并带有版本、解析与来源字段。CSV 答案表与题目 ID 顺序核对一致；Markdown 审核稿只用于人工审核，不参与运行时加载。

只有同时满足 `enabled === true`、`verified === true` 且通过结构校验的题目才能进入考试。系统只验证数据结构，不核实、生成或自动修正 LOL 答案。以后新增或修改题目仍必须由人工提供真实题目、答案、解析与来源后审核。
